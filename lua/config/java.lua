...
local function open_terminal(title, cwd, argv)
	...
	vim.fn.jobstart(argv, {
		...
		on_exit = function(_, code)
			notify(
				string.format("%s finished with exit code %d", title, code),
...
local function java_bundles()
	...
	if debug_path then
		vim.list_extend(
			bundles,
			vim.fn.glob(
				vim.fs.joinpath(debug_path, "extension", "server", "com.microsoft.java.debug.plugin-*.jar"),
				true,
				true
			)
		)
...
local function lombok_jar()
	...
	local function maven_jar()
		local jars = vim.fn.glob(vim.fs.joinpath(vim.fn.expand("~/.m2"), "repository", "org", "projectlombok", "lombok", "**", "lombok-*.jar"), true, true)
		if #jars == 0 then
			return nil
		end
		table.sort(jars)
...
local function set_buffer_keymaps(bufnr)
	...
	map("n", "<leader>jgg", function()
		jdtls.extract_variable_all()
	end, "Extract all occurrences")
...
function M.setup_maven_commands()
	...
	vim.api.nvim_create_user_command("MvnArchetype", function()
		...
		input_sequence({
			...
			{
				key = "archetype_catalog",
				prompt = "Archetype Catalog",
...
function M.attach_jdtls(bufnr)
	...
	-- 1. Find the actual Equinox launcher jar inside Mason
	local jdtls_path = mason_package_path("jdtls")
	local launcher_jar = vim.fn.glob(vim.fs.joinpath(jdtls_path, "plugins", "org.eclipse.equinox.launcher_*.jar"), true, true)[1]

	-- 2. Figure out your OS configuration folder name
	...
	-- 3. Construct a completely raw Java execution command
	local cmd = {
	...
	-- 4. Inject Lombok strictly right before the launcher
	if lombok then
		table.insert(cmd, "-javaagent:" .. lombok)
	end

	-- 5. Append the required Equinox framework arguments
	vim.list_extend(cmd, {
		"-jar", launcher_jar,
		"-configuration", vim.fs.joinpath(jdtls_path, os_config),
		"-data", workspace_dir(root)
	})
	local config = {
		cmd = cmd,
		root_dir = root,
