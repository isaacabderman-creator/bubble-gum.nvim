...
local function lombok_jar()
  ...
  local function maven_jar()
    local jars = vim.fn.glob(
      vim.fs.joinpath(vim.fn.expand("~/.m2"), "repository", "org", "projectlombok", "lombok", "**", "lombok-*.jar"),
      true,
      true
    )
    if #jars == 0 then
...
function M.attach_jdtls(bufnr)
  ...
  local jdtls_path = mason_package_path("jdtls")
  local launcher_jar =
    vim.fn.glob(vim.fs.joinpath(jdtls_path, "plugins", "org.eclipse.equinox.launcher_*.jar"), true, true)[1]
